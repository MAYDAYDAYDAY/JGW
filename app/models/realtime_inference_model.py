...
from app.models.desktop_billing import can_afford_desktop_batch, charge_desktop_after_saves
...
class RealtimeInferenceModel(QObject):
    ...
    def _count_saveable_session_results(self) -> int:
        return sum(
            1
            for r in self.inference_results
            if r is not None and getattr(r, "original_image", None) is not None
        )
    ...
    def save_inference_session(self):
        ...
        n = self._count_saveable_session_results()
        ok, msg = can_afford_desktop_batch(self.model_name, n)
        if not ok:
            self.inference_error_signal.emit(msg)
            return False
    ...
    def _save_session_to_db(self):
        ...
        try:
            for result in self.inference_results:
                ...
                saved_ids.append(inference_result_id)
            ...
            # 发出数据更新事件
            if success_count > 0:
                bill_ok, bill_msg = charge_desktop_after_saves(self.model_name, saved_ids)
                if not bill_ok and bill_msg:
                    EventBus.get_instance().message_notification.emit("桌面扣费", bill_msg)

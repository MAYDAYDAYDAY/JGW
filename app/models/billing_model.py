...
class BillingModel(QObject):
    ...
    api_calls_updated = pyqtSignal(object)  # {'total': int, 'items': list}
    ...
    def _load_api_calls_task(self, page, page_size, signals):
        """加载API调用记录的后台任务（含总数，供分页）"""
        total = self.billing_dao.count_all_api_calls()
        items = self.billing_dao.get_all_api_calls(page, page_size)

...
class InferenceModel(QObject):
    ...
    def start_inference(self, model_name: str, images: List[np.ndarray]):
        ...
        ok, msg = can_afford_desktop_batch(model_name, len(images))
        if not ok:
    ...
    def save_2_db_task(self, results: List[InferenceResult], signals: WorkerSignals):
        ...
        for i, result in enumerate(results):
            try:
                ...
                if not model_name_for_bill and getattr(result, "model_name", None):
                    model_name_for_bill = result.model_name
        ...
        # 发送数据更新信号
        if success_count > 0:
            if model_name_for_bill and saved_ids:
                bill_ok, bill_msg = charge_desktop_after_saves(model_name_for_bill, saved_ids)
